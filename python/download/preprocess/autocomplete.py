...
def char_vocabulary(chars, vocab_size):
    vocabulary_index = np.concatenate([
        np.array([
...
def make_source_target_alignment(words, word_map, char_map, max_length,
                                 verbose=False):
    space_char_code = char_map[' ']
    unknown_word_code = word_map['<unknown>']
    ...
    for word in tqdm(words, disable=not verbose):
        ...
        target_current.append(
            np.full(len(word) + 1,
                    word_map[word] if word in word_map else unknown_word_code,
                    dtype='int32')
        )
...
def build_dataset(text,
                  vocab_size=2**14, max_length=200, verbose=False, **kwargs):
    if verbose:
    ...
    word_map = word_vocabulary(words, vocab_size=vocab_size)
    char_map = char_vocabulary(chars, vocab_size=vocab_size)

    if verbose:
        print('making dataset ...')
    length, source, target = make_source_target_alignment(
        words, word_map, char_map, max_length=max_length,
        verbose=verbose
    )

    return {
        'word_map': export_map(word_map),
